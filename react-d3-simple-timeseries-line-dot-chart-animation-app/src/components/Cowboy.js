...
const Cowboy = ({data, maxHeight = 300}) => {
  console.log(data);
  ...
  const columns = useMedia({
    ...
    values: [
      ...
      {tick: d3.timeMonday, fmt: `%a %m/%d`},
      {tick: d3.timeWednesday, fmt: `%a %m/%d`},
      {tick: d3.timeSunday, fmt: `%a %m/%d`},
    ],
    defaultValue: {tick: d3.timeSunday, fmt: `%a %m/%d`},
    // values: [5, 4, 3, 2],
    // defaultValue: 2,
  });
