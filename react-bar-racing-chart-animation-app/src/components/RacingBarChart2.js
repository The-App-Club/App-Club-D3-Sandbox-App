import {useRef, useEffect} from 'react';
import * as d3 from 'd3';
import {css} from '@emotion/css';
import {useResizeObserver} from '../hooks/useResizeObserver';
import {formatCount} from '../plugins/count';
const RacingBarChart2 = ({data, maxHeight = 300}) => {
  const svgDomRef = useRef();
  const graphDomRef = useRef();
  const dimensions = useResizeObserver({ref: graphDomRef});

  useEffect(() => {
    const svg = d3.select(svgDomRef.current);
    svg.attr('height', maxHeight);
    if (!dimensions) {
      return;
    }

    // sorting the data
    data.sort((a, b) => {
      // return b.value - a.value; //desc
      return a.value - b.value; //asc
    });

    const yScale = d3
      .scaleLinear()
      .domain([
        0,
        d3.max(data, (d) => {
          return d.value;
        }),
      ])
      .range([0, dimensions.height]);

    const xScale = d3
      .scaleBand()
      .domain(
        data.map((d, i) => {
          return i;
        })
      )
      .range([0, dimensions.width])
      .padding(0.1);

    // draw the bars
    svg
      .selectAll('.bar')
      .data(data, (d, i) => {
        return d.name;
      })
      .join(
        (enter) =>
          enter.append('rect').attr('x', (d, i) => {
            return xScale(i);
          }),
        (update) => update,
        (exit) => exit.remove()
      )
      .attr('fill', (d, i) => {
        return d.color;
      })
      .attr('class', 'bar')
      .attr('width', (d) => {
        return xScale.bandwidth();
      })
      .transition()
      .ease(d3.easeCubicInOut)
      .attr('y', (d) => {
        return dimensions.height - yScale(d.value);
      })
      .attr('height', (d) => {
        return yScale(d.value);
      })
      .attr('x', (d, i) => {
        return xScale(i);
      });

    svg
      .selectAll('.point')
      .data(data, (d, i) => {
        return d.name;
      })
      .join(
        (enter) =>
          enter.append('text').attr('x', (d, i) => {
            return xScale(i) + xScale.bandwidth() / 2 - 8;
          }),
        (update) => update,
        (exit) => exit.remove()
      )
      .text((d, i) => {
        return `${formatCount({count: d.value, base: 10})}`;
      })
      .attr('class', 'point')
      .transition()
      .ease(d3.easeCubicInOut)
      .attr('y', (d, i) => {
        return dimensions.height - yScale(d.value) - 5;
      })
      .attr('x', (d, i) => {
        return xScale(i) + xScale.bandwidth() / 2 - 8;
      });
  }, [data, dimensions, maxHeight]);

  return (
    <div ref={graphDomRef} className={css``}>
      <svg
        ref={svgDomRef}
        className={css`
          background: hsl(0, 100%, 100%);
          overflow: visible;
          display: block;
          width: 100%;
        `}
      ></svg>
    </div>
  );
};

export {RacingBarChart2};
